...
const rawgApiKey = "79fef868b7604333b57ebde17d0922a8";
...
(() => {
	try {
		...
		app.get("/api/topRatedRecommended", async (req, res) => {
			...
			const topRatedRandomVideogame =
				topRatedResponse.results[randomIndex];
			res.set("Cache-Control", "no-cache");
		...
		app.get("/api/videogame/:rawgId", async (req, res) => {
					...
					compatibilityOneyObj = [
						{
							...
							thumbnails: {
								medium: { url: oneyObj[0].yt_thumbnail },
							},
						},
					];
				}
			...
			const detailsCollected = {
				...primary,
				screenshots:
					parseInt(primary.screenshots_count) > 0
						? secondary[0].results
						: [],
				reviews:
					parseInt(primary.reviews_count) > 0
						? secondary[1].results
						: [],
				devteam:
					parseInt(primary.creators_count) > 0
						? secondary[2].results
						: [],
		...
		app.get("/api/searchArchive", async (req, res) => {
			try {
				...
				console.log(
					`/api/searchArchive?title=${queryTitle}&year=${queryYear} endpoint has been called!`
				);
			} catch (e) {
				console.error(e);
			}
		...
		app.get("/api/searchOldgameshelf", async (req, res) => {
			try {
				...
				console.log(
					`/api/searchOldgameshelf?title=${queryTitle} endpoint has been called!`
				);
			} catch (e) {
				console.error(e);
			}
		...
		app.get("/api/searchSnesnow", async (req, res) => {
			try {
				...
				console.log(
					`/api/searchSnesnow?title=${queryTitle} endpoint has been called!`
				);
			} catch (e) {
				console.error(e);
			}
		...
		app.get("/api/videogameAutocomplete", async (req, res) => {
			...
			console.log(
				`/api/videogameAutocomplete?q=${query} endpoint has been called!`
			);
		});
